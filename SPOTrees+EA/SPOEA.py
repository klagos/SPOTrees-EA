...
# block print for the gurobi thingy
# Disable
def blockPrint():
...
seed = int(sys.argv[9])
dimensions = int(sys.argv[10])

hc1 = float(sys.argv[11])
hc2 = float(sys.argv[12])
inq = float(sys.argv[13])
deq = float(sys.argv[14])
newpercTotal = float(sys.argv[15])
...
setDimencionAndSeed(dim, seed, hc1, hc2, inq, deq, newpercTotal)
#############################################################################
#############################################################################
#############################################################################
assert(reps_st >= 0)
...
# Esta parte lo que hace es retornar el optimo de la primera observacion
'''
print("RESOLVIENDO SOLO ............")
train_x,train_cost,test_x,test_cost = data[200][2][0][0]
cost = train_cost[0,:]
print(shortest_path(cost)["objective"])
print("*"*10)
'''
...
for deg in deg_set:
  ...
  for trial_num in range(reps_st,reps_end):
    print data[n_train][deg][eps][trial_num]
    ...
    arch = open("time_dim_"+ str(dim) + "_depth_" + str(max_depth) +"_noise_"+ str(eps) +".txt", "a")
    f = "Deg {0}+Trial {1}+ Error {2}\n"
    arch.write(f.format(deg, trial_num, str(end-start)))
    arch.close()
    print "Elapsed time: " + '\033[92m\033[4m' + str(end-start) + '\033[0m'
    ...
    opt_decision = find_opt_decision_real(test_cost)['weights']
    ...
    incurred_test_cost = np.sum(costs_deg[deg][trial_num])
    opt_test_cost = np.sum(optcosts_deg[deg][trial_num])

    #percent error:
    print("SPO Tree: Test Set Normalized SPO Loss: ")
    SPO_error = 100.0*(incurred_test_cost-opt_test_cost)/opt_test_cost
    totalError += SPO_error
    numberOfErrors += 1
    arch = open("error_dim_"+ str(dim) +"_depth_" + str(max_depth) +"_noise_" +str(eps) +".txt", "a")
    f = "Deg {0}+Trial {1}+ Error {2}\n"
    arch.write(f.format(deg, trial_num, SPO_error))
    arch.close()
    print('\033[92m\033[4m' + str(SPO_error)+'\033[0m'+" percent error")
...
#print(costs_deg[2])
#print("*"*10)
#print(optcosts_deg[2])

print totalError/numberOfErrors
