...
for deg in deg_set:
  ...
  for trial_num in range(reps_st,reps_end):
    ...
    arch = open("time_dim_"+ str(dim) + "_depth_"+ str(max_depth)+ "_noise_" + str(eps) +"_original.txt", "a")
    ...
    arch = open("error_dim_"+ str(dim) + "_depth_" + str(max_depth) + "_noise_"+ str(eps) +"_original.txt", "a")
    f = "Deg {0}+Trial {1}+ Error {2}\n"
...
'''
filen = open(fname_out, 'rb')
data = pickle.load(filen)
filen.close()
cnt = 0
for item in data:
  print('The data predicted is', cnt, ' is:', item)
  cnt += 1

filen = open(fname_out_opt, 'rb')
data = pickle.load(filen)
filen.close()
cnt = 0
for item in data:
  print('The data optimal is', cnt, ' is:', item)
  cnt += 1
'''
